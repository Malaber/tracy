...
def test_ci_and_release_workflows_publish_expected_tags():
    ...
    assert "python -m invoke browser-e2e" in ci
    assert "passkey-browser-e2e" in ci
...
def test_webhooker_bundle_matches_published_image_contract():
    ...
    review_compose = Path("deploy/webhooker/compose.review.yml").read_text(encoding="utf-8")
    ...
    for compose in (production_compose, review_compose):
        assert "APP_BASE_URL: https://${APP_HOSTNAME}" in compose
        assert "WEBAUTHN_RP_ID: ${APP_HOSTNAME}" in compose
        assert "SECRET_KEY: ${TRACY_SECRET_KEY}" in compose
